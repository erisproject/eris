...
class Matrix {
    public:
        ...
        /** Returns a matrix view of the first `rows` rows of this matrix. */
        Matrix top(unsigned int rows);

        /** Returns a matrix view of the first `rows` rows of this matrix. */
        Matrix top(unsigned int rows) const;

        /** Returns a matrix view of the last `rows` rows of this matrix. */
        Matrix bottom(unsigned int rows);

        /** Returns a matrix view of the last `rows` rows of this matrix. */
        Matrix bottom(unsigned int rows) const;

        /** Returns a matrix view of the first `cols` columns of this matrix. */
        Matrix left(unsigned int cols);

        /** Returns a matrix view of the first `cols` columns of this matrix. */
        Matrix left(unsigned int cols) const;

        /** Returns a matrix view of the last `cols` columns of this matrix. */
        Matrix right(unsigned int cols);

        /** Returns a matrix view of the last `cols` columns of this matrix. */
        Matrix right(unsigned int cols) const;
        ...
        /** Scales a matrix by the given scalar value, returning the result in a new matrix.
         */
        friend Matrix operator*(double d, const Matrix &b);
...
class Vector : public Matrix {
    public:
        ...
        /** Returns a view of this vector containing just the first `n` elements. */
        Vector head(unsigned int n);

        /** Returns a view of this vector containing just the first `n` elements. */
        Vector head(unsigned int n) const;

        /** Returns a view of this vector containing just the last `n` elements. */
        Vector tail(unsigned int n);

        /** Returns a view of this vector containing just the last `n` elements. */
        Vector tail(unsigned int n) const;
...
class RowVector : public Matrix {
    public:
        /// Default constructor that creates a null matrix; see Matrix().
        RowVector();
        /** Creates a RowVector from a Matrix.  Throws an exception if the Matrix is not a row matrix.
         */
        ...
        /** Returns a view of this row vector containing just the first `n` elements. */
        RowVector head(unsigned int n);

        /** Returns a view of this row vector containing just the first `n` elements. */
        RowVector head(unsigned int n) const;

        /** Returns a view of this row vector containing just the last `n` elements. */
        RowVector tail(unsigned int n);

        /** Returns a view of this row vector containing just the last `n` elements. */
        RowVector tail(unsigned int n) const;
...
inline Matrix& Matrix::operator-=(const Matrix &b) { impl() -= b.impl(); return *this; }
inline Matrix Matrix::operator*(const Matrix &b) const { return impl() * b.impl(); }
inline Matrix Matrix::operator*(double d) const { return impl() * d; }
inline Matrix Matrix::operator/(double d) const { return impl() * (1.0/d); }
inline Matrix operator*(double d, const Matrix &b) { return b.impl() * d; }
inline Matrix& Matrix::operator*=(double d) { impl() *= d; return *this; }
inline Matrix& Matrix::operator/=(double d) { impl() *= (1.0/d); return *this; }
...
inline std::ostream& operator<<(std::ostream &os, const Matrix &A) { return os << (std::string) A; }
...
inline Matrix Matrix::top(unsigned int nrows)          { return block(0, 0, nrows, cols(), const_); }
inline Matrix Matrix::top(unsigned int nrows) const    { return block(0, 0, nrows, cols(), true); }
inline Matrix Matrix::bottom(unsigned int nrows)       { return block(rows()-nrows, 0, nrows, cols(), const_); }
inline Matrix Matrix::bottom(unsigned int nrows) const { return block(rows()-nrows, 0, nrows, cols(), true); }
inline Matrix Matrix::left(unsigned int ncols)         { return block(0, 0, rows(), ncols, const_); }
inline Matrix Matrix::left(unsigned int ncols) const   { return block(0, 0, rows(), ncols, true); }
inline Matrix Matrix::right(unsigned int ncols)        { return block(0, cols()-ncols, rows(), ncols, const_); }
inline Matrix Matrix::right(unsigned int ncols) const  { return block(0, cols()-ncols, rows(), ncols, true); }

inline Vector Vector::head(unsigned int n)       { return block(0,        0, n, 1, const_); }
inline Vector Vector::head(unsigned int n) const { return block(0,        0, n, 1, true); }
inline Vector Vector::tail(unsigned int n)       { return block(cols()-n, 0, n, 1, const_); }
inline Vector Vector::tail(unsigned int n) const { return block(cols()-n, 0, n, 1, true); }

inline RowVector RowVector::head(unsigned int n)       { return block(0,        0, 1, n, const_); }
inline RowVector RowVector::head(unsigned int n) const { return block(0,        0, 1, n, true); }
inline RowVector RowVector::tail(unsigned int n)       { return block(0, cols()-n, 1, n, const_); }
inline RowVector RowVector::tail(unsigned int n) const { return block(0, cols()-n, 1, n, true); }
}
