...
// PositionalAgent and CircularPosAgent.
...
TEST(Construction, Unbounded) {
    PositionalAgent p1({1});
    CircularPosAgent p2({2.5});
...
TEST(Construction, Bounded) {
    PositionalAgent  p1a({1}, {3}, {0});
    PositionalAgent  p1b({1}, {0}, {3});
    PositionalAgent  p1c({1}, {INF}, {0});
    PositionalAgent  p1d({1}, {-INF}, {3});
    PositionalAgent  p1e({3}, {-INF}, {3});
    PositionalAgent  p1f({0}, {0}, {3});
    CircularPosAgent p2a({1}, {3}, {0});
    CircularPosAgent p2b({1}, {0}, {3});
    CircularPosAgent p2c({1}, {INF}, {0});
    CircularPosAgent p2d({1}, {-INF}, {3});
...
TEST(Boundaries, Boundaries) {
    PositionalAgent p1({1}, {3}, {0});
    EXPECT_THROW(p1.moveBy({-2}), PositionalAgent::boundary_error);

    PositionalAgent p2({1}, {3}, {0});
    EXPECT_THROW(p2.moveBy({2.01}), PositionalAgent::boundary_error);

    PositionalAgent p3({1}, {3}, {0});
    ...
    CircularPosAgent p4({1,2,3,4,5}, {0,0,0,0,0}, {5,5,5,5,5}, {});
    EXPECT_NO_THROW(p4.moveTo({4,1,2,5,0}));
    EXPECT_TRUE(p4.binding());
    EXPECT_TRUE(p4.bindingUpper());
    EXPECT_TRUE(p4.bindingUpper());

    EXPECT_THROW(p4.moveBy({1.00001,0,0,0,0}), PositionalAgent::boundary_error);
    EXPECT_THROW(p4.moveBy({0,0,0,0.00000000001,0}), PositionalAgent::boundary_error);
    EXPECT_THROW(p4.moveBy({0,-1.0001,0,0,0}), PositionalAgent::boundary_error);
...
TEST(Wrapping, Circle) {
    CircularPosAgent p1({1}, {-1}, {5});

    EXPECT_NO_THROW(p1.moveTo({19}));
    EXPECT_EQP({1}, p1.position());

    CircularPosAgent p2({1}, {-1.25}, {5.25});
...
TEST(Wrapping, Donut) {
    CircularPosAgent p1({1,1}, {-2,-2}, {3,3}); // Wrap in both dimensions
    ...
    CircularPosAgent p2({2.25, 0}, {-10,-10}, {10,10});
...
TEST(Wrapping, Cylinder) {
    ...
    CircularPosAgent p1({1,1}, {-5,-5}, {-1,-1}, {1});
    ...
    EXPECT_THROW(p1.moveBy({99,0}), PositionalAgent::boundary_error);
