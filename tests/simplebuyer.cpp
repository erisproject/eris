...
//         NB: SimpleBuyer can't actually handle this well, because each
...
TEST(Case06_Numeraire, Px1_Py1_I69) {
    ...
    // allow SimpleBuyer to get within numerical precision error of 3.95/2.95.
    con->assets() = 6.9*m1;
    int rounds2 = 0;
    SimpleBuyer opt2(con, m, 138);
    opt2.reset();
...
/* The below, with different prices, doesn't find the optimum, not because SimpleBuyer is broken,
 ...
 * correct).
 *
 * This is not really a bug, however: that's how SimpleBuyer is *supposed* to work: hence "simple".
 */
