 ...
 * suitable, but any callable object can be used.
 *
 ...
 * two cdfs and one cdf complement, or one cdf and two complements).  If this is specified, exactly
 * two cdf calls are used always.
 *
 ...
 * be calculated from probabilities.  The default is 0.3, but that is likely a suboptimal value for
 * any particular distribution and/or generator.
 *
 ...
 * default is 0 (i.e. don't do pre-cdf calculation draws at all), which is suitable for any
 * distribution with relative fast cdf calculations.  Setting this to a positive value can make a
 * substantial difference for other distributions (such as chi-squared), when the truncation range
 * isn't in an extremely unlikely part of the distribution support.
 *
...
auto truncDist(
    ...
    ResultType min,
    ResultType max,
    ResultType median = std::numeric_limits<ResultType>::quiet_NaN(),
