 ...
 *
 * This implementation is submitted to boost (but not yet included, as of boost 1.61):
 * https://github.com/boostorg/random/pull/23; the only difference here is that this is moved into
 * the eris::random namespace instead of boost::random namespace.
 *
...
#include <eris/random/detail/int_float_pair.hpp>
...
struct unit_normal_distribution
{
    ...
    RealType generate_tail(Engine& eng) {
        ...
        exponential_distribution<RealType> exp_x(tail_start);
        exponential_distribution<RealType> exp_y;
        for(;;) {
