...
public:
    ...
    /** Constructs a market that provides units of output_unit in exchange for units of price_unit.
     * All exchanged quantities will be in scalar multiples of these bundles.
     */
    ...
    struct quantity_info {
        /** Default construct: yields a quantity_info with `.constrained` set to false, and numeric
         * values set to 'nan'.
         */
        quantity_info() = default;

        /** Constructor for a set of quantities; initializes quantity, constrained, spent, unspent
         * to the given values.
         */
        ...
        /** Constructor for an unconstrained purchase; `quantity` and `spent` are set to the given
         * values, `constrained` is set to false, and `unspent` is set to 0.
         */
        ...
        /** The amount (in multiples of the market's price Bundle) of unspent income.  Exactly equal
         * to .price - input_price.  Will be 0 when .constrained is false.
         */
        double unspent = std::numeric_limits<double>::quiet_NaN();
    };

    /** Contains a reservation of market purchase.  The market will consider the reserved quantity
     * unavailable until a call of either buy() (which completes the transfer) or release() (which
     * cancels the transfer) is called.
     *
    ...
    private:
        ...
        // Temporary bundle that holds the payment when the reservation is constructed and either
        // returns it to the agent (if cancelled) or gives it to firms (if completed).
        Bundle b_;
    ...
    public:
        ...
        // need to const_cast away the constness on the move source (otherwise copy constructors get
        // invoked, which leaves market set, which breaks destruction).
        ...
        /** Reserves the given BundleNegative transfer from the given firm and stores the result, to
         * be transferred if buy() is called, and aborted if release() is called.  Positive amounts
         * are to be transferred from the firm, negative amounts are to be transferred to the firm.
         * This is intended to be called only by Market subclasses.
         */
        ...
        /** Exception class thrown if attempting to buy or release a Reservation that has already
         * been bought or released.
         */
    ...
    /** Returns the price information for buying the given multiple of the output bundle.  Returned
     * is a price_info struct with .feasible set to true iff the quantity can be produced in this
     * market.  If feasible, .total, .marginal, and .marginalFirst are set to the total price and
     * marginal prices of the given quantity.  .marginal is the marginal price of the last
     * infinitesimal unit (which is not necessarily the same as the marginal price for the *next*
     * unit sold, as the quantity could just hit a threshold where marginal price jumps).
     * .marginalFirst is the marginal price of the very first (fractional) unit produced.
     * Essentially, marginalFirst should not depend on the quantity requested (but often will depend
     * on past market purchases), and won't change until some internal economy state changes (such
     * change need not occur in this market, however: transactions in other markets may affect the
     * participating firms of this market).
     *
     * Often .marginalFirst <= .marginal will hold, but it doesn't have to: a supplier could, for
     * example, have increasing returns to scale.
     *
     ...
     * price Bundle will purchase.  The information is returned in a quantity_info struct, which has
     * two important fields:
     * - .quantity the quantity that can be bought for the given price
     * - .constrained will be true if the provided price would violate a supply constraint in this
     *   market.  In such a case .quantity is the constrained amount and .spend contains the actual
     *   amount spent (which will be less than the input amount).  Note that it is possible for
     *   .constrained to be false even when not all of p is spent: see below.
     ...
     *
     * .spent and .unspent may differ from the description above in a market of discrete goods.  For
     * example, a market setting integer increments of x for $2 each called with quantity(13.8)
     * should give {.quantity=6, .constrained=false, .spent=12, .unspent=1.8}.
     */
     ...
     * provided Agent.  When the reservation is completed, the amount is transfered to the firm; if
     * aborted, the amount is returned to the Agent's assets Bundle.  Returns a Reservation object
     * that should be passed in to buy() to complete the transfer, or release() to abort the sale.
     *
     * \param q the quantity to reserve, as a multiple of the market's output_unit.
     * \param agent a SharedMember<Agent> (or Agent subclass) from whose assets Bundle the payment
     * will be taken.  The removed amount will be held until either buy() or release() is called, at
     * which point it will be transferred to the seller(s) or returned to the Agent, respectively.
     * \param p_max the maximum price to pay, as a multiple of the market's price_unit.  Optional;
     * if omitted, defaults to infinity (i.e. no limit).
     *
     * This method is intended to be called in the optimize() method of an IntraOptimizer instance,
     * stored, and completed via buy() in the Optimizer's apply() method.
     *
     ...
     * when assets are less than p_max*price: the actual transaction price could be low enough that
     * assets is sufficient.
     */
    virtual Reservation reserve(SharedMember<Agent> agent, double q, double p_max = std::numeric_limits<double>::infinity()) = 0;

    /** Completes a reservation made with reserve().  Transfers the reserved assets into the assets
     * Bundle of the Agent supplied when creating the reservation, and transfers the reserved
     * payment to the firm(s) supplying the output.
     *
     * The provided Reservation object is updated to record that it has been purchased.
     *
     * \throws Market::Reservation::non_pending_exception if the given reservation has already been
...
public:
    /** Adds f to the firms supplying in this market.  Subclasses that require a particular type of
     * firm should override this method, calling `requireInstanceOf<Base>(f, "...")` followed by
     * `Market::addFirm(f)`; see eris::market::Bertrand as an example.
     */
    virtual void addFirm(SharedMember<Firm> f);

    /** Removes f from the firms supplying in this market.  This is called automatically if a firm
     * passed to addFirm is removed from the simulation; manual calling is only needed if a firm
     * exits the market but stays in the simulation. */
    ...
    /** Exception class thrown when a quantity that exceeds the market capacity has been requested.
     */
    ...
    /** Exception class throw when the market price required exceeds the maximum price specified.
     */
    ...
    /** Exception class thrown when the buyer's assets are not sufficient to purchase the requested
     * output.
     */
    ...
    /** The base unit of this market's price; prices paid for output are in terms of multiples of
     * this Bundle.  Subclasses are not required to make use of this bundle.
     */
...
protected:
    ...
    /** Overridden to automatically remove a firm from the market when the firm is removed from the
     * simulation.
     */
