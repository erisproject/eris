 ...
 * the firm actually carry out production; that occurs in QFirm's interAdvance() method.
 *
 ...
 * capacity (in which case we jump back to the last period's sales).
 *
...
class QFStepper : public ProfitStepper {
    public:
         ...
         * \param profit_basis the basis Bundle for measuring profits.  Typically something like
         * Bundle({money, 1}), but could also be something more exotic such as `Bundle({leftShoe,
         * 12}, {rightShoe, 12})`, which would measure profit in terms of dozens of pairs of shoes.
         * \param step the initial (relative) step size
